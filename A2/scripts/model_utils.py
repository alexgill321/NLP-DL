...
from state import Token, ParseState, is_final_state, shift, left_arc, right_arc, is_legal, find_children
...
def train_loop_dep(model, train_loader, dev_data, lr, c=2, emb=('840B', 300), save_dir=None):
    name, d_emb = emb
    w_emb = torchtext.vocab.GloVe(name=name, dim=d_emb)
    ...
    for ep in range(1, max_epochs+1):
        ...
        train_loss = []
        train_acc = []
        for w, p, dep, lab in tqdm(train_loader):
            model.train()
            optimizer.zero_grad()
            w = w.to(device)
            p = p.to(device)
        ...
        for tokens, actions in tqdm(dev_data):
            ...
            while not is_final_state(state, c):
                ...
                w_list = [w for w in w]
                w = w_emb.get_vecs_by_tokens(w_list, lower_case_backup=True) 
                w = w.view(1, 4*c, d_emb)
                w = w.to(device)
                p = [[pos_tags[pos] for pos in p]]
                p = torch.tensor(p).to(device)
                l = [[label_tags[label] for label in l]]
